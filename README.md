...
| 데스크톱      | 아이콘 선택·이동·정렬, 그리드 맞춤, 아이콘 표시 끄기, 이름 변경, 우클릭 메뉴, 새로 만들기(폴더·문서·인터넷 바로 가기)                                                                                                       |
| 시작 메뉴     | 앱 목록, 앱·파일 검색(실제 폴더 경로 표시), 파일 내용 검색, 앱 고정·해제, 전원 메뉴                                                                                                                                         |
| 작업 표시줄   | 실행 앱 표시, 다중 창 개수·미리보기, 우클릭 점프 리스트(최근 항목), 최소화·복원, 바탕 화면 보기, 화면 네 가장자리 중 원하는 곳에 배치, 자동 숨기기, 작은 단추                                                               |
...
| 시스템 UI     | 시스템 트레이, 달력(일정 추가·삭제, 시간 되면 알림), 알림 센터, 실행 창, 작업 표시줄 검색·우클릭 메뉴, 전환 효과                                                                                                            |
| 클립보드      | 모든 창이 공유하는 하나의 클립보드, 복사·잘라내기·붙여넣기                                                                                                                                                                  |
| 설정          | 테마, 배경, 작업 표시줄 위치·자동 숨기기·작은 단추, 야간 조명, 소리, 기본 앱, 사용자 이름, 24시간제 시계, 텍스트 크기, 집중 지원, 폴더 옵션(파일 확장명·숨긴 항목)                                                          |
| 파일 시스템   | IndexedDB 기반 폴더 계층(문서·사진·게임·다운로드), 이동·복사, 이름 충돌 시 바꾸기·건너뛰기·둘 다 유지, 연결 앱, 트리 단위 휴지통, ZIP 백업, 셸 전체가 공유하는 실행 취소·다시 실행                                          |
...
| 파일 탐색기       | 독립된 여러 창, 접었다 펴는 탐색 창 폴더 트리, 실제 폴더 경로, 뒤로·앞으로·위로, 새 폴더·문서, 드래그 이동과 Ctrl 복사, 정렬·열 너비 조정·속성, 하위 폴더와 파일 내용까지 검색, 주소 표시줄 하위 폴더 이동, 실행 취소 |
| 휴지통            | 파일과 폴더 트리 복원, 영구 삭제, 휴지통 비우기                                                                                                                                                                       |
| Microsoft Edge    | 주소 이동, 검색, 방문 기록, 창 안 웹 보기, 읽기 보기, 페이지 다운로드(읽기 보기는 Markdown, 웹 보기는 .url), 차단 페이지 복구                                                                                         |
| 메모장            | 여러 문서, 자동 저장, Windows형 열기·다른 이름으로 저장, 찾기·바꾸기, Markdown 미리보기                                                                                                                               |
| 그림판            | 브러시, 지우개, 도형, 스포이드, 캔버스 크기 조정, Windows형 PNG 열기·저장                                                                                                                                                              |
| 사진              | 이미지 보기, 이전·다음, 슬라이드 쇼, 확대·축소, 회전, 이름 바꾸기                                                                                                                                                     |
...
| 캡처 도구         | 끌어서 고른 사각형·화면·활성 창을 PNG로 찍어 사진 폴더에 저장 (PrintScreen, Alt+PrintScreen, Win+Shift+S)                                                                                                                                |
...
| `Win/⌘ + 1~9`           | 작업 표시줄 n번째 앱 (없으면 실행, 여러 창이면 차례로)    |
| `Win/⌘ + Shift + 1~9`   | 그 앱의 새 창                                             |
| `Win/⌘ + M`             | 모든 창 최소화                                            |
| `Win/⌘ + L`             | 잠금                                                      |
| `Ctrl + C` / `X` / `V`  | 복사 · 잘라내기 · 붙여넣기 (바탕 화면과 탐색기 공통)      |
| `Ctrl + A`              | 전체 선택                                                 |
| `F2`                    | 선택한 파일 또는 아이콘 이름 변경                         |
| `F2` (여러 개 선택)     | 한 이름으로 번호를 붙여 일괄 이름 변경                    |
| `Delete`                | 선택 항목을 휴지통으로 이동                               |
| `Shift + Delete`        | 휴지통을 거치지 않고 완전히 삭제 (확인 후)                |
| `Ctrl + Z` / `Ctrl + Y` | 파일 작업 실행 취소 · 다시 실행 (바탕 화면과 탐색기 공통) |
| `Ctrl + Shift + N`      | 탐색기에서 새 폴더                                        |
| `Alt + Enter`           | 선택 항목 속성                                            |
| `Alt + D`               | 탐색기 경로 입력 (Ctrl + L과 같음)                        |
| `Ctrl/⌘ + 휠`           | 탐색기 보기 전환 (큰 아이콘 ↔ 목록 ↔ 자세히)              |
| 가운데 클릭             | 폴더를 새 탭에서 열기                                     |
| `Ctrl/⌘ + 드래그`       | 이동 대신 복사                                            |
...
| `Win + V`               | 클립보드 기록                                             |
| `Win + .` / `Win + ;`   | 이모지 패널                                               |
| `Win + X`               | 작업 표시줄 메뉴 (파워 유저 메뉴)                          |
| `Ctrl + F` / `Ctrl + E` / `F3` | 탐색기 검색 상자로 이동                            |
| `Ctrl + G`              | 메모장 줄 이동                                            |
| `Ctrl + 휠`             | 메모장 확대/축소 (`Ctrl + 0` 기본 크기)                   |
...
아홉 단계를 순서대로 실행하며, 하나라도 실패하면 멈춥니다.
...
| `qa:a11y`       | 앱 17개와 셸 표면 전체의 ARIA 구조, 창 안에 머무는 포커스               |
| `qa:pwa`        | 서비스 워커 설치, 핵심 번들 사전 캐시와 실제 오프라인 재실행            |

버전을 태그할 때는 한 단계 더 실행합니다.

```bash
npm run qa:release
```

`qa:all` 뒤에 `audit:runtime --strict`를 붙입니다. 레지스트리가 죽어 있으면
평소 게이트는 "확인하지 못했다"고 적고 통과하지만(장애는 보안 결과가 아니므로),
태그를 찍는 순간에는 그렇게 넘어가면 안 됩니다.
...
- [x] 접근성 게이트 (`qa:a11y`) — 앱 17개·메뉴·패널의 ARIA 구조를 CI에서 검사
- [x] 창 단위 문서 라벨 — 같은 앱의 창들이 폴더·페이지·경로로 구분
- [x] 성능 — 드래그 중 동기 I/O 제거·쓰기 코얼레싱, 창 슬롯 memo(비대상 창 리렌더 0, 스모크가 `data-shared-props`로 검사), 스티커 메모 키스트로크당 쓰기 제거(14자에 18회 → 1회), 상호작용 중 블러 정지
- [x] 코드 스플리팅 — 앱 17개가 각자 청크(첫 창을 열 때 로드, 데스크톱까지 588KB → 366KB), 읽기 보기 Markdown 스택 지연 로드, 서비스 워커가 빌드 자산 목록으로 청크까지 프리캐시
- [x] 스티커 메모 앱, 그림 파일 아이콘 썸네일, 바탕 화면 클릭 시 창 포커스 해제, 창 미리보기가 실제 창 축소판, Aero Peek·Aero Shake, 작업 표시줄 메뉴의 창 배열, 작업 보기 드래그로 데스크톱 이동, 탐색기 세부 정보 창 텍스트 미리보기, 진짜 스크린샷(PrintScreen·캡처 도구), 그림 파일을 바탕 화면 배경으로 설정, 작업 표시줄 버튼으로 접히고 펴지는 최소화·복원, 전원 메뉴 절전, 클릭 가능한 Alt+Tab, 잠금 화면에도 내 배경, 찬 휴지통 아이콘, 창 축소판이 보이는 스냅 미리보기·화면 나누기 후보, 그림이 보이는 알림·시작 메뉴 추천, 알람 있는 날을 표시하는 트레이 달력, 경로 입력이 되는 탐색기 주소 표시줄, 검색어 강조, 선택 크기 표시, 클릭으로 열리는 알림, 끌어서 바꾸는 고정 앱 순서, 알림 하나씩 지우기, 폴더의 새 창·명령 프롬프트 열기, 탐색기 다중 탭, 시작 메뉴 타일 폴더, 설정의 단축키 목록, 그림판 도형 채우기, 실측 메모리, 탐색기 탭 키·드래그, 시작 메뉴 폴더 이름, 시계 우클릭 메뉴, 탭 화살표 이동과 가운데 클릭으로 탭 닫기, 압축(ZIP) 파일로 압축·압축 풀기, 하위 폴더까지 검색, 빠른 액세스 폴더 고정, 스냅 레이아웃(Win+Z, 3분할·4분할), 가상 데스크톱 이름 바꾸기, 텍스트 크기 100~~150%, 집중 지원, 작업 표시줄 위치(왼쪽·위쪽·오른쪽·아래쪽 — 작업 영역·스냅·아이콘 격자·플라이아웃이 함께 이동), 트레이 달력 일정(종일·시각 지정, 시간 되면 알림, 놓친 일정 표시), 폴더 속성의 실제 크기·내용 개수, 숨김 특성과 숨긴 항목 보기, 파일 확장명 표시, 드라이브 속성(사용량 원형 차트), 작업 표시줄 자동 숨기기·작은 단추, 야간 조명(세기 조절), 그림판 텍스트 도구, 탐색기 그룹화(유형·수정한 날짜), 보내기(바탕 화면·문서·사진·ZIP), 셸 전체가 공유하는 파일 작업 실행 취소·다시 실행, 파일 바꾸기 또는 건너뛰기, 접었다 펴는 탐색 창 폴더 트리, 파일 내용 검색(탐색기·시작 메뉴), 주소 표시줄 하위 폴더 이동, Ctrl 드래그 복사, 탐색기 열 너비 조정, 메모장 바꾸기(Ctrl+H), Win+1~~9 작업 표시줄 앱, 바탕 화면 아이콘 표시 끄기, Ctrl+휠 보기 전환, 가운데 클릭으로 새 탭, Shift+Delete 영구 삭제, 창 위에 놓은 파일을 그 창이 열기, 사진 슬라이드 쇼, 이름 충돌 거절, 여러 항목 속성과 선택 크기, 여러 항목 이름 바꾸기, 그림판 스포이드·크기 조정, 탐색기 끌어서 선택, 메모장 확대/축소·줄 이동, 휴지통에 끌어다 버리기·바탕 화면에서 비우기, 클립보드 기록(Win+V), 탐색기 검색 단축키·상태 표시줄 보기 전환·인터넷 바로 가기 만들기, 이모지 패널(Win+.), 시작 메뉴 항목 메뉴(열기·시작/작업 표시줄 고정), 캡처 도구 사각형 캡처, 바탕 화면 Ctrl+휠 아이콘 크기·작업 표시줄 Shift+클릭 새 창·Win+X
