...
- [x] 스티커 메모 앱, 그림 파일 아이콘 썸네일, 바탕 화면 클릭 시 창 포커스 해제, 창 미리보기가 실제 창 축소판, Aero Peek·Aero Shake, 작업 표시줄 메뉴의 창 배열, 작업 보기 드래그로 데스크톱 이동, 탐색기 세부 정보 창 텍스트 미리보기, 진짜 스크린샷(PrintScreen·캡처 도구), 그림 파일을 바탕 화면 배경으로 설정, 작업 표시줄 버튼으로 접히는 최소화
