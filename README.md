...
| 데스크톱      | 아이콘 선택·이동·정렬, 그리드 맞춤, 이름 변경, 우클릭 메뉴, 새로 만들기(폴더·문서·인터넷 바로 가기)                                                                                                                         |
| 시작 메뉴     | 앱 목록, 앱·파일 검색(실제 폴더 경로 표시), 앱 고정·해제, 전원 메뉴                                                                                                                                                         |
| 작업 표시줄   | 실행 앱 표시, 다중 창 개수·미리보기, 우클릭 점프 리스트(최근 항목), 최소화·복원, 바탕 화면 보기                                                                                                                             |
| 창 관리자     | 이동, 8방향 크기 조절, 최소화, 최대화, 절반·사분면 스냅, 화면 나누기 후보, 앱별 다중 창, 바탕 화면 클릭 시 창 비활성, 작업 표시줄·Alt+Tab·작업 보기의 실제 창 축소판, Aero Peek, Aero Shake, 계단식 배열·위아래/나란히 정렬 |
...
| 캡처 도구         | 화면·활성 창을 PNG로 찍어 사진 폴더에 저장 (PrintScreen, Alt+PrintScreen, Win+Shift+S)                                        |
| 계산기            | 일반·공학 계산과 키보드 입력                                                                                                  |
| 스티커 메모       | 메모마다 하나의 창, 5색, 첫 줄이 창 제목, 창을 닫아도 메모는 남고 새로고침 후 제자리 복원                                     |
...
- [x] 코드 스플리팅 — 읽기 보기 Markdown 스택 지연 로드, 서비스 워커가 빌드 자산 목록으로 청크까지 프리캐시
- [x] 스티커 메모 앱, 그림 파일 아이콘 썸네일, 바탕 화면 클릭 시 창 포커스 해제, 창 미리보기가 실제 창 축소판, Aero Peek·Aero Shake, 작업 표시줄 메뉴의 창 배열, 작업 보기 드래그로 데스크톱 이동, 탐색기 세부 정보 창 텍스트 미리보기, 진짜 스크린샷(PrintScreen·캡처 도구), 그림 파일을 바탕 화면 배경으로 설정, 작업 표시줄 버튼으로 접히고 펴지는 최소화·복원, 전원 메뉴 절전, 클릭 가능한 Alt+Tab, 잠금 화면에도 내 배경, 찬 휴지통 아이콘
