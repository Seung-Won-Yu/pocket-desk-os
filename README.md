...
| 창 관리자     | 이동, 8방향 크기 조절, 최소화, 최대화, 절반·사분면 스냅, 화면 나누기 후보, 앱별 다중 창, 바탕 화면 클릭 시 창 비활성, 작업 표시줄·Alt+Tab·작업 보기의 실제 창 축소판, Aero Peek, 계단식 배열·위아래/나란히 정렬 |
...
- [x] 스티커 메모 앱, 그림 파일 아이콘 썸네일, 바탕 화면 클릭 시 창 포커스 해제, 창 미리보기가 실제 창 축소판, Aero Peek, 작업 표시줄 메뉴의 창 배열
